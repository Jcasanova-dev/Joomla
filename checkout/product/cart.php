...
<script type="text/javascript">
	window.Oby.registerAjax(<?php echo $events; ?>, function(params) {
		var o = window.Oby,
			el = document.getElementById('hikashop_cart_<?php echo $module_id; ?>');
		if (!el) return;
		...
		window.hikashop.xRequest("<?php echo hikashop_completeLink('product&task=cart&module_id=' . $module_id . '&module_type=' . $this->cart_type . $this->url_itemid, true, false, true); ?>", {
			update: false,
			mode: 'POST',
			data: 'return_url=<?php echo urlencode(base64_encode(hikashop_currentURL('return_url'))); ?>'
		}, function(xhr) {
...
<?php
	...
	if ($this->params->get('print_cart', 0)) {
		$print_button = $this->popup->display(
			...
			'HIKA_PRINT',
			hikashop_completeLink('cart&task=printcart&cid=' . $this->element->cart_id, true),
			'hikashop_print_popup',
			760,
			480,
			'title="' . JText::_('HIKA_PRINT') . '"',
			'',
			'link'
		);
	}
...
foreach ($columns as $c) {
	...
	<form action="<?php echo hikashop_completeLink('product&task=updatecart' . $this->url_itemid, false, true); ?>" method="post" name="hikashop_<?php echo $this->element->cart_type; ?>_form" onsubmit="if(window.hikashop) return window.hikashop.submitCartModule(this, 'hikashop_cart_<?php echo $module_id; ?>', '<?php echo $this->element->cart_type; ?>');">
		<table class="hikashop_cart" width="100%">
			...
			<tbody>
				...
				$k = 0;
				$same_product_parent_id = false;
				$previous_product_parent_id = null;
				$printed_before_colon = false;
				$printed_before_description = false;
				$total_products = 0;
				$grouped_products = array();
				$conteo_product_parent_id = array();
				$ultimo_conteo_por_product_parent_id = array();
				$sum_cantidad_por_product_parent_id = array();
				$sum_valor_por_product_parent_id = array();
				foreach ($this->element->products as $product) {
					...
					$group_key = ($product_parent_id == 0) ? 'independent_' . $product->cart_product_id : $product_parent_id;
					$grouped_products[$group_key][] = $product;
					$previous_product_parent_id = $product_parent_id;
				...
				foreach ($grouped_products as $grouped_product) {
					...
					$sum_cantidad_por_product_parent_id = array();
					$sum_valor_por_product_parent_id = array();
					foreach ($grouped_product as $product) {
						$product_parent_id = $product->product_parent_id;
						$product_price_numeric = floatval(trim($product->product_msrp));
						$product_msrp_total = $product_price_numeric * $product->cart_product_quantity;

						if (isset($conteo_product_parent_id[$product_parent_id])) {
							$conteo_product_parent_id[$product_parent_id]++;
							$sum_cantidad_por_product_parent_id[$product_parent_id] += $product->cart_product_quantity;
							$sum_valor_por_product_parent_id[$product_parent_id] += $product_msrp_total;
						} else {
							$conteo_product_parent_id[$product_parent_id] = 1;
							$sum_cantidad_por_product_parent_id[$product_parent_id] = $product->cart_product_quantity;
							$sum_valor_por_product_parent_id[$product_parent_id] = $product_msrp_total;
						}
					}

					foreach ($conteo_product_parent_id as $product_parent_id => $conteo) {
						$ultimo_conteo_por_product_parent_id[$product_parent_id] = $conteo;
					}
					$k = 0;
					foreach ($grouped_product as $product) {
						$row_counter++;
				?>
						<tr class="row<?php echo $k; ?>" id="product-row-<?php echo $product->product_parent_id . '-' . $row_counter; ?>" data-product-parent-id="<?php echo $product->product_parent_id; ?>">
							<!-- IMAGE -->
							<?php
							if (!empty($columns['image']) && !$image_shown) {
							?>
								<td class="hikashop_cart_module_product_image hikashop_cart_value" style="vertical-align:middle !important;text-align:center;">
									...
									?>
										<img class="hikashop_product_cart_image" title="<?php echo $this->escape((string)@$product->images[0]->file_description); ?>" alt="<?php echo $this->escape((string)@$product->images[0]->file_name); ?>" src="<?php echo $img->url; ?>" <?php echo $attributes; ?> />
									<?php
									} else {
									?>
									...
									$image_shown = true; // Marcar la imagen como mostrada
									?>
								</td>
							<?php
							} elseif (!empty($columns['image'])) {
							?>
								<td style="width: <?php echo $width; ?>px; height: <?php echo $height; ?>px;"></td>
							<?php
							}
							?>
							<!-- EO IMAGE -->
							...
							?>
								<td class="hikashop_cart_module_product_name_value hikashop_cart_value">
									...
									?><a href="<?php echo hikashop_contentLink('product&task=show&cid=' . $product->product_id . '&name=' . $product->alias . $this->url_itemid, $product); ?>"><?php
										...
										$partBeforeColon = isset($parts[0]) ? $parts[0] : '';
										$partAfterColon = isset($parts[1]) ? $parts[1] : '';
										if ($product_parent_id != $previous_product_parent_id) {
											$printed_before_colon = false;
										}
										if (!$printed_before_colon) {
										?>
											<div class="product-name-before">
												<?php echo $partBeforeColon; ?>
												<!-- Mostrar la cantidad de productos para el product_parent_id actual -->
												<?php echo ' (' . $ultimo_conteo_por_product_parent_id[$product_parent_id] . ' Variantes, ' . $sum_cantidad_por_product_parent_id[$product_parent_id] . ' Cant ) '; ?>
											</div>
											<style>
												...
												@media screen and (max-width: 700px) {
													.product-name-before {
														font-size: 8px;
														/* Tamaño de letra para dispositivos con ancho de pantalla menor a 700px */
													}
										...
										<?php
											$printed_before_colon = true;
										}

										?>

										<div class="product-name-after"><?php echo $partAfterColon; ?></div>
										<?php
										if ($link_to_product == 1) {
										?>
										</a><?php
							...
							?>
								<td class="hikashop_cart_module_product_quantity_value hikashop_cart_value">
									...
									echo $this->loadHkLayout('quantity', array(
										...
										'onchange_script' => 'validateQuantity(this);',
										'onincrement_script' => 'validateQuantity(this);'
									));
									?>
								</td>
							<?php
				...
				<?php
						$previous_product_parent_id = $product->product_parent_id;
						$k = 1 - $k;
					}
					$conteo_product_parent_id = array();
					$sum_cantidad_por_product_parent_id = array();
	...
	?>
		<a class="checkout-button <?php echo $css_button . ' ' . $css_button_checkout; ?>" href="<?php echo $this->url_checkout; ?>" onclick="if(this.disable) return false; this.disable = true;">
			<span><?php echo JText::_('PROCEED_TO_CHECKOUT'); ?></span>
		</a>
		<script>
			function validateQuantity(input) {
				var partAfterColon = "<?php echo $partAfterColon; ?>";
				var braValue = partAfterColon.match(/BRA \d+/)[0].split(' ')[1]; // Extrae el valor BRA
				...
				var multiplesOf5 = ["33", "34", "35", "36", "37", "45", "46", "47"];
				var multiplesOf10 = ["38", "39", "40", "41", "42", "43", "44"];

				if (multiplesOf5.includes(braValue) && quantity % 5 !== 0) {
					alert('Debe ingresar múltiplos de 5 o 10.');
					input.value = '';
					return;
				}

				if (multiplesOf10.includes(braValue) && quantity % 10 !== 0) {
					alert('Debe ingresar múltiplos de 5 o 10.');
					input.value = '';
					return;
				}
				...
				if (input.form.onsubmit && !input.form.onsubmit()) return;
				input.form.submit();
	...
	<!-- BOTTOM EXTRA DATA -->
	<?php
	if (!empty($this->extraData->bottom)) {
