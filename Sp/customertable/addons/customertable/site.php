...
class SppagebuilderAddonCustomertable extends SppagebuilderAddons
{
    public function render()
    {
        ...
        foreach ($existingRecords as $record) {
            ...
            $output .= '<a href="https://mwt.one/index.php?option=com_sppagebuilder&view=page&id=27&customer_id=' . $record['customer_id'] . '" class="btn btn-primary">Actualizar</a>';
            $output .= '</td>';
            $output .= '</tr>';
        }
    ...
    public function getUploadedFiles($recordIdToDelete) {
        try {
            $db = JFactory::getDbo();
            $query = $db->getQuery(true);
    ...
    public function deletecustomeruser($recordIdToDelete) {
        $db = JFactory::getDbo();
        try {
            $filePathRelative = $this->getUploadedFiles($recordIdToDelete);
