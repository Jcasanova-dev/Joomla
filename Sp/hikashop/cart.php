		...
		if (!empty($columns['image']) && !$image_shown) {
			...
			<td class="hikashop_cart_module_product_image hikashop_cart_value"
				...
				if ($img->success) {
					...
					?>
					<img class="hikashop_product_cart_image"
						 title="<?php echo $this->escape((string)@$product->images[0]->file_description); ?>"
						 alt="<?php echo $this->escape((string)@$product->images[0]->file_name); ?>"
						 src="<?php echo $img->url; ?>" <?php echo $attributes; ?>/>
					<?php
				} else {
					?>
		...
		if (!empty($columns['name'])) {
			...
			<td class="hikashop_cart_module_product_name_value hikashop_cart_value">
				...
				if ($link_to_product == 1) {
					?><a
					href="<?php echo hikashop_contentLink('product&task=show&cid=' . $product->product_id . '&name=' . $product->alias . $this->url_itemid, $product); ?>"><?php
				}
				?>
				<?php
				...
				if (!$printed_before_colon) {
					?>
					<div class="product-name-before" style="font-size: 18px;">
						<?php echo $partBeforeColon; ?>
						<!-- Mostrar la cantidad de productos para el product_parent_id actual -->
						<?php echo ' (' . $ultimo_conteo_por_product_parent_id[$product_parent_id] . ' Variants, ' . $sum_cantidad_por_product_parent_id[$product_parent_id] . ' Qty, ' . $sum_valor_por_product_parent_id[$product_parent_id] . ' Subtotal)'; ?>
					</div>
					<?php
					$printed_before_colon = true;
				}
				if (!$same_product_parent_id && !$printed_before_description && isset($product->product_description)) {
					?>
					<div class="product-description"><?php echo $this->escape((string)$product->product_description); ?></div>
					<?php
					$printed_before_description = true;
				}
		...
		if (!empty($columns['delete'])) {
			...
			<td class="hikashop_cart_module_product_delete_value hikashop_cart_value">
				<a href="<?php echo $delete_url; ?>"
				   data-cart-id="<?php echo (int)$this->element->cart_id; ?>"
				   data-cart-type="<?php echo $this->escape($this->element->cart_type); ?>"
				   data-cart-product-id="<?php echo (int)$product->cart_product_id; ?>"
				   onclick="if(window.hikashop) { return window.hikashop.deleteFromCart(this, null, 'hikashop_cart_<?php echo $module_id; ?>'); }"
