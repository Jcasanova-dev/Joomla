...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $originalData = array(
        ...
        'clase' => $_POST['data']['plugin']['plugin_params']['clase'][0]
    );
        ...
        foreach ($_POST['data']['plugin']['plugin_params']['word_to_replace'] as $key => $value) {
            if ($key != 0) {
                $duplicatedData[] = array(
                    ...
                    'clase' => $_POST['data']['plugin']['plugin_params']['clase'][$key]
                );
            }
        }
    }
            ...
            <div>
                ...
                if ($pluginParamsData) {
                    ...
                    if (is_array($pluginParamsData->word_to_replace)) {
                        foreach ($pluginParamsData->word_to_replace as $index => $word) {
                            $params = array(
                                ...
                                'clase' => $pluginParamsData->clase[$index]
                            );
                            $allParams[] = $params;
                        }
                    } else {
                    ...
                    foreach ($allParams as $index => $params) {
                        ...
                        echo '<dd><textarea id="descripcion_' . $index . '" name="data[plugin][plugin_params][descripcion][]">' . $this->escape($params['descripcion']) . '</textarea></dd>';
                        echo '<script src="/media/vendor/tinymce/tinymce.min.js" defer></script>';
                        echo '<script>
                            document.addEventListener("DOMContentLoaded", function() {
                                tinymce.init({
                                    selector: "#descripcion_' . $index . '",
                                    height: 250,
                                    plugins: "link image code",
                                    toolbar: "undo redo | bold italic | alignleft aligncenter alignright | code",
                                    content_css: [
                                        "https://mwt.one/templates/system/css/general.css",
                                        "https://mwt.one/templates/system/css/print.css"
                                    ]
                                });
                            });
                        </script>';

                        echo '<dt>' . JText::_('Clase') . '</dt>';
                        echo '<dd><select id="class_field_' . $index . '" name="data[plugin][plugin_params][clase][]">';
                        echo '<option value="Segmentos"' . ($params['clase'] === 'Segmentos' ? ' selected="selected"' : '') . '>Segmentos</option>';
                        echo '<option value="Riesgos"' . ($params['clase'] === 'Riesgos' ? ' selected="selected"' : '') . '>Riesgos</option>';
                        echo '<option value="Manejo de Energías"' . ($params['clase'] === 'Manejo de Energías' ? ' selected="selected"' : '') . '>Manejo de Energías</option>';
                        echo '<option value="Otros"' . ($params['clase'] === 'Otros' ? ' selected="selected"' : '') . '>Otros</option>';
                        echo '</select></dd>';
                        echo '</dl>';
                    }
                } else {
                    ...
                    echo '<dt>' . JText::_('Clase') . '</dt>';
                    echo '<dd><select id="class_field_' . $index . '" name="data[plugin][plugin_params][clase][]">';
                    echo '<option value="Segmentos">Segmentos</option>';
                    echo '<option value="Riesgos">Riesgos</option>';
                    echo '<option value="Manejo de Energías">Manejo de Energías</option>';
