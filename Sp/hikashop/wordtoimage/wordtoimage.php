    ...
    private function replaceWordsWithImages(&$content, $pluginParams)
    {
        ...
        if (is_array($wordToReplace)) {
            foreach ($wordToReplace as $index => $word) {
                ...
                if (!isset($clase[$index])) {
                    $clase[$index] = '';
                }
                $pattern = '/(<input\s+[^>]*\btype\s*=\s*["\']?hidden["\']?[^>]*>|<(a|li|select|option)[^>]*>.*?<\/\2>(*SKIP)(*FAIL))|' . preg_quote($word, '/') . '(?!\.png)/i';
                $content = preg_replace_callback(
                    $pattern,
                    function ($matches) use ($word, $imageToInsert, $link, $index, $language, $descripcion, $clase) {
                        if (!empty($matches[1])) {
                            return $matches[1];
                        }
                        $wordpage = $word;
